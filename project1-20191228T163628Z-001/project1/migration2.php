...
$query = "SELECT max_cpu FROM vm_table where vm_id=3";
$query1 = "SELECT vm_memory FROM vm_table where vm_id=3";
$query2 = "SELECT storage FROM vm_table where vm_id=3";
...
if( $storage*2 <=  $sub)
{
	if($memory*2<= $sub1)
	{
$update_s = "UPDATE vm_table SET max_cpu = $cpu , vm_memory = $memory*2, storage = $storage*2 WHERE vm_id = 3 ";
$r_s = mysqli_query($dbconnect,$update_s);
}
else{
$queryupdate = "UPDATE vm_table SET max_cpu = $cpu , vm_memory = $memory , storage = $storage WHERE vm_id = 20 ";
