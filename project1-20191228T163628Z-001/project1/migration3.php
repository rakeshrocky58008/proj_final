...
$query = "SELECT max_cpu FROM vm_table where vm_id=4";
$query1 = "SELECT vm_memory FROM vm_table where vm_id=4";
$query2 = "SELECT storage FROM vm_table where vm_id=4";

$total_s = "SELECT storage FROM cluster_table WHERE cluster_id = 1";
$total_m = "SELECT total_memory FROM cluster_table WHERE cluster_id = 1";
...
$sum = "SELECT sum(storage) from vm_table where esxi_id in ( select esxi_id from esxi_table where cluster_id =1)";
$sum1 = mysqli_query($dbconnect,$sum);
$sum2 = mysqli_fetch_assoc($sum1);
$value_av = $sum2["sum(storage)"];


$sum_mem = "SELECT sum(vm_memory) from vm_table where esxi_id in ( select esxi_id from esxi_table where cluster_id =1)";
...
if( $storage*2 <=  $sub)
{
	if($memory*2<= $sub1)
	{
$update_s = "UPDATE vm_table SET max_cpu = $cpu , vm_memory = $memory*2, storage = $storage*2 WHERE vm_id = 4 ";
$r_s = mysqli_query($dbconnect,$update_s);
}
else{
$queryupdate = "UPDATE vm_table SET max_cpu = $cpu , vm_memory = $memory , storage = $storage WHERE vm_id = 21 ";
